...
findstr "eslint.*8" package.json >nul
if %ERRORLEVEL% EQU 0 (
    echo ✅ ESLint 8 настроен (совместимо с eslint-config-next)
) else (
    echo ❌ ESLint 8 не настроен
    exit /b 1
)

echo.
echo 3. Проверка отсутствия проблемных модулей...
findstr "react-indiana-drag-scroll" package.json >nul
if %ERRORLEVEL% EQU 0 (
    echo ❌ react-indiana-drag-scroll все еще в зависимостях
    exit /b 1
) else (
    echo ✅ react-indiana-drag-scroll удален из зависимостей
)

echo.
echo 4. Очистка и переустановка зависимостей...
if exist node_modules rmdir /s /q node_modules
if exist .next rmdir /s /q .next
call npm install --legacy-peer-deps

echo.
echo 5. Финальная сборка...
call npm run build
...
echo ✅ Конфликт версий ESLint (ESLint 8 вместо ESLint 9)
echo ✅ Отсутствующий модуль react-indiana-drag-scroll (заменен на CSS)
